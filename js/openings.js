...
function clearBoard(context) {
  ...
  for (let i=0;i<8;i++)
    for (let j=0;j<8;j++) {
      context.fillStyle = ((i + j)%2 ? COLORS.dark : COLORS.light) 
...
class Piece {
  constructor(...args) {
    if (args.length == 1) {
      if (/^[kqrbnp]$/i.test(args[0]))
        this.letter = args[0];
    } else if (args.length == 2) {
      //if (typeof args[0] == 'symbol' && typeof args[1] == 'symbol')
      throw 'NYI';
    }
  }
  get type() {
...
if (STORAGE.getItem('lines'))
  lines = JSON.parse(STORAGE.getItem('lines'))
else 
  lines = [];

const MINIMAL_INTERVAL = 60_000; // 60s (one minute)
...
document.getElementById('messages').innerHTML = '<p>EDITING MODE</p><p>Make moves on the board by dragging pieces. Drag an empty square towards the right to add the line to the database.</p>';
...
function save() {
  STORAGE.setItem("lines", JSON.stringify(lines));
}
function load() {
  lines = JSON.parse(STORAGE.getItem("lines"));
}
          ...
          try {
            ...
            switch (operatingMode) {
              ...
              case EDITING:
                moves.push(move);
                document.getElementById('pgn').innerHTML =
                  moves.map(m => (m.position.turn == WHITE ? m.position.movenumber + '.' : '') + m.fan).join(' ');
              default:
                sounds.move.play();
            }
        ...
        if (Math.floor(emptySquareDrag[1].x) - Math.floor(emptySquareDrag[0].x) > BOARD_SIZE/8) {
          if (moves.length > 0) {
            ...
            if (!lines.includes(line)) {
              ...
              lines.push(line);
            } else console.warn('line already exists');
            moves = [];
            position = new Position();
            draw();
            document.getElementById('pgn').innerHTML = '';
...
function pickALine() {
  // https://chat.openai.com/c/c8eea46b-b043-422e-88ec-120db55b8e28
  // https://gist.github.com/TheHans255/7a072162ec171e72f766ea954c5f11b3
  let coefficients = [], sum = 0, now = new Date();
  for (let line of lines) {
    let data = JSON.parse(STORAGE.getItem(line)),
      nextDate = new Date(
        new Date(data.lastAttempt).valueOf()
        + MINIMAL_INTERVAL*Math.pow(INTERVAL_MULTIPLIER, data.streak)
      ),
      coeff = now.valueOf() - nextDate.valueOf();
    sum += coefficients.push(Math.exp(coeff/MINIMAL_INTERVAL));
  }
  let sampler = alias_sampler(coefficients);
