...
function canPremove(state, orig, dest) {
  const validPremoves = state.premovable.customDests?.get(orig) ?? premove(state.pieces, orig, state.premovable.castle);
...
function configure(state, config) {
  ...
  if (!state.movable.rookCastle && state.movable.dests) {
    ...
    state.movable.dests.set(
      kingStartPos,
      dests.filter(
        (d) => !(d === "a" + rank && dests.includes("c" + rank)) && !(d === "h" + rank && dests.includes("g" + rank))
      )
    );
  }
}
  ...
  requestAnimationFrame(() => {
    ...
    if (!origPiece || !samePiece(origPiece, cur.piece))
      ...
      if (cur.started) {
        ...
        cur.keyHasChanged ||= cur.orig !== getKeyAtDomPos(cur.pos, whitePov(s), bounds);
      }
    }
    processDrag(s);
  });
